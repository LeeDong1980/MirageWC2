...
namespace MirageSDK.Provider
{
	public static class MirageSDKFactory
	{
        ...
        public static IMirageSDK GetMirageSDKInstance(string providerURI, WCSignClient _wc, bool autoSetup = false)
        {
            return CreateMirageSDKInstance(providerURI, autoSetup, _wc);
        }
		...
		private static IMirageSDK CreateMirageSDKInstance(string providerURI, bool autoSetup, WCSignClient _wc = null)
		{
			if (autoSetup)
			{
			...
			var eth = new Mobile.EthHandler(web3Provider, silentSigningHandler, _wc);
		#endif
