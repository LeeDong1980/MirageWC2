...
namespace MirageSDK.SilentSigning.Implementation
{
	public class SilentSigningProtocol : ISilentSigningHandler
	{
        ...
        private WalletConnectSharp.Unity.WalletConnect WalletConnect; //no need to use with WCv2
        [Inject]
        ...
        {
            var data = new SilentSigningConnectionRequest(timestamp, chainId);
            var (session, _, _) = GetCurrentAddress();
            var requestSilentSign = await _wc.Request<SilentSigningConnectionRequest, SilentSigningResponse>(session.Topic, data);
                //await WalletConnect.Send<SilentSigningConnectionRequest, SilentSigningResponse>(data);
        ...
        {
            var (session, _, _) = GetCurrentAddress();
            var response = await _wc.Request<TRequest, SilentSigningResponse>(session.Topic, request);
            if (response.IsError)
            {
                switch (response.Error.Code)
                {
